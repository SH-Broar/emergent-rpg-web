...
const rules = computed(() => ({
  combat: ['빈 칸을 누르면 상하좌우로 걸어갑니다. 한 칸은 30초입니다.', '물건이나 인물이 있는 칸을 누르면 가까이 가서 바라봅니다. 길 끝은 다음 장소로 이어집니다.', '필드 마물은 피할 수 있습니다. 붉은 칸은 다음 공격 위치입니다. 던전 안에는 엘리트와 보스가 기다립니다.'],
  colors: ['도형은 같은 원리로 작동하지만 대상과 손에 든 재료에 따라 결과가 달라집니다.', '↑ ↓는 물건의 위치, ← →는 재료의 흐름, △ ▽는 대상의 상태, ○는 교류에 관여합니다.', '같은 도형을 반복해서 익히면 해당 도형의 힘·운반력·효율이 자랍니다. 잘못 그렸을 때는 시간이 흐르지 않습니다.'],
...
function draw(g: Gesture) {
  ...
  const expected = ui.tutorialTopic === 'colors' ? ['up', 'down', 'triangle'] : ui.tutorialTopic === 'life' ? ['inverted', 'circle', 'left'] : ['circle', 'circle'];
...
<template>
  <dialog ref="dialog" class="tutorial" aria-labelledby="tutorial-title" @close="close" @cancel="close" @click="event => { if (event.target === dialog) close(); }">
    <div class="tutorial-body"><header><h2 id="tutorial-title">작은 연습</h2><button autofocus aria-label="튜토리얼 닫기" @click="close">×</button></header>
      ...
      <GesturePad v-else compact @gesture="draw" @unrecognized="message = '한 번 더 그려보세요.'"/>

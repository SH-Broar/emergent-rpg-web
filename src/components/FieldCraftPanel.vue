...
<template><div v-if="open" class="craft-backdrop" @click.self="emit('close')"><section role="dialog" aria-modal="true" aria-label="도구 가공"><header><strong>가공 <small>{{ station?.name??'작업대 가까이에서' }}</small></strong><button aria-label="가공 닫기" @click="emit('close')">×</button></header><article v-for="recipe in SUPPLY_RECIPES" :key="recipe.id"><div><strong>{{ recipe.name }} ×{{ Object.values(recipe.output)[0] }}</strong><p>{{ recipe.effect }}</p><small class="recipe-cost"><span v-for="[id,n] in Object.entries(recipe.input)" :key="id" :class="{missing:(player.stock[id]??0)<n}">{{ fieldItemName(id) }} {{ player.stock[id]??0 }}/{{ n }} </span></small></div><button :disabled="!station||!enough(recipe.input)" @click="make(recipe.id)">만들기</button></article><p class="hint">손에 고른 뒤 ╲ 그리기</p><p role="status">{{ notice }}</p></section></div></template>
<style scoped>.craft-backdrop{position:fixed;inset:0;background:#102019da;z-index:955;display:grid;place-items:center;padding:12px;color:#e5debc}.craft-backdrop section{width:min(100%,430px);max-height:88dvh;overflow:auto;background:#182a22;border:1px solid #68715a;border-radius:12px;padding:16px;box-sizing:border-box}header,article{display:flex;align-items:center;justify-content:space-between;gap:10px}header{margin-bottom:12px}header small{display:block;color:#a7b8a3;font-size:11px;margin-top:4px}article{padding:14px 0;border-top:1px solid #70826844}article strong{font-size:14px}p{font-size:12px;color:#c0cab5;margin:6px 0}small{font-size:11px;color:#9eaf96}.recipe-cost{display:flex;gap:4px 10px;flex-wrap:wrap}.recipe-cost span{white-space:nowrap}.missing{color:#c79280}button{min-height:38px;border:1px solid #8a9474;border-radius:4px;background:#344a38;color:#f0e3bd;padding:5px 12px;font:inherit}button:disabled{opacity:.4}.hint{text-align:center;color:#dacb95}</style>

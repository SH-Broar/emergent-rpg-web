...
import { paletteStyle } from '@/systems/field-palette';
...
const imminent=computed(()=>new Set(entities.value.flatMap(e=>e.creature?.pending?.remaining===1&&e.creature.nextAction?.kind!=='wait'?e.creature.intent??[]:[]).map(positionKey)));
...
const layeredTargets = computed(() => fieldTargets(at(selection.value)));
...
async function clickCell(pos: GridPos) {
  ...
  if(candidates.filter(e=>e.id!=='player'&&e.kind!=='terrain').length>1){detailsOpen.value=true;return;}
...
<template>
  <main v-if="initialized && space && player" class="field-view" :style="paletteStyle(baseNode(run.data),space)" @pointerdown.capture="press" @keydown.capture="wake">
...
.field-stage { flex: 1; min-height: 0; position: relative; display: flex; align-items: center; justify-content: center; overflow: hidden; background: radial-gradient(ellipse, var(--field-stage), var(--field-edge)); padding: 8px; box-sizing: border-box; }
...
.tile--grass { background: var(--field-grass); }.tile--grass:nth-child(3n) { background: var(--field-grassAlt); }.tile--path { background: var(--field-path); }.tile--soil { background: repeating-linear-gradient(165deg, #725e44 0 7px, #66533e 7px 9px); }.tile--water { background: var(--field-water); }.tile--stone { background: var(--field-stone); }.tile--wall { background: var(--field-wall); box-shadow: inset 0 -7px #15281f66; }.dungeon .tile--wall { background: #303735; box-shadow: inset 0 -7px #171c1ccc; }
...
.tile--sand{background:var(--field-sand)}.tile--wood{background:repeating-linear-gradient(90deg,#0000 0 13px,#0002 13px 15px),var(--field-wood)}

...
const SCENERY_POOL: Record<string, string[]> = {
  Wilderness:      ['황야에 바람이 세차게 분다.', '멀리서 야생 동물의 울음소리가 들린다.', '풀숲 사이로 작은 짐승이 스쳐 지나간다.', '광활한 하늘 아래 발걸음이 가벼워진다.'],
  Mountain_Path:   ['험준한 산길을 오른다. 숨이 가빠진다.', '바위 틈에서 신선한 샘물이 흘러내린다.', '안개가 자욱해 앞이 잘 보이지 않는다.', '정상에서 부는 바람이 땀을 식혀준다.'],
  Lake:            ['수면 위로 물새 한 마리가 날아오른다.', '맑은 호수에 하늘이 비친다.', '물안개가 주변을 부드럽게 감싼다.'],
  Herb_Garden:     ['향긋한 풀내음이 코를 간지럽힌다.', '이슬에 젖은 허브잎이 빛을 받아 반짝인다.'],
  Trade_Route:     ['행상인의 마차가 반대 방향으로 지나간다.', '길가에 오래된 이정표가 서 있다.', '먼지가 자욱한 도로 위를 묵묵히 걷는다.'],

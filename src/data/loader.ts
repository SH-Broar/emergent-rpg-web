...
function parseOneCard(id: string, f: IniSection): Card | null {
  ...
  return {
    ...
    effects,
    magic: Object.keys(f).some(key => key.startsWith('magic_')) ? {
      family: f.magic_family, strokes: f.magic_strokes ? parseNumber(f.magic_strokes, 2) : undefined,
      glyphs: f.magic_glyphs ? parseList(f.magic_glyphs) : undefined,
      mana: f.magic_mana !== undefined ? parseNumber(f.magic_mana, 1) : undefined,
      cooldown: f.magic_cooldown !== undefined ? parseNumber(f.magic_cooldown, 2) : undefined,
      effects: f.magic_effects ? parseList(f.magic_effects).map(parseCardEffect).filter((e): e is CardEffect => e !== null) : undefined,
      role: f.magic_role,
    } : undefined,

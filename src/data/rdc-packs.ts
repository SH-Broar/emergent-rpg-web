// rdc-packs.ts — RDC 캐릭터팩 정의 및 해금 로직
// 팩 해금 조건: conditionMembers 캐릭터 전원 히페리온 Lv.5 달성.
// 해금 후 playableNames 캐릭터가 캐릭터 선택 화면에 추가됨.
// NPC 등장 여부(DataPackConfig)와는 완전히 독립된 시스템.

import type { Actor } from '../models/actor';
import { getGlobalSave, unlockRdcPack } from './global-save';

export interface RdcPackDef {
  id: string;
  label: string;
  description: string;
  /** 히페리온 Lv.5 조건 캐릭터 목록 */
  conditionMembers: string[];
  /** 팩 해금 시 플레이어블이 되는 캐릭터 목록 */
  playableNames: string[];
}

export const RDC_PACKS: RdcPackDef[] = [
  {
    id: 'chapter0',
    label: 'Chapter. 0',
    description: '이유르, 리엔카이, 모노',
    conditionMembers: ['이유르', '리엔카이', '모노'],
    playableNames:    ['이유르', '리엔카이', '모노'],
  },
  {
    id: 'spiritz',
    label: 'Spiritz',
    description: '임페리시아, 발렌시아, 아카샤',
    conditionMembers: ['임페리시아', '발렌시아', '아카샤'],
    playableNames:    ['임페리시아', '발렌시아', '아카샤'],
  },
  {
    id: 'cheonri',
    label: '천리지각',
    description: '카요, 하피 시이드, 네토 로크',
    conditionMembers: ['카요', '하피 시이드', '네토 로크'],
    playableNames:    ['카요', '하피 시이드', '네토 로크'],
  },
  {
    id: 'celestial',
    label: 'Celestial Wings',
    description: '리제, 리비트, 피닉스',
    conditionMembers: ['리제', '리비트', '피닉스'],
    playableNames:    ['리제', '리비트', '피닉스'],
  },
  {
    id: 'holiday',
    label: 'Holiday',
    description: '루디, 카르디, 테오',
    conditionMembers: ['루디', '카르디', '테오'],
    playableNames:    ['루디', '카르디', '테오'],
  },
  {
    id: 'luna3',
    label: '루나 3학년',
    description: '마로 엔야, 이연, 페비엘',
    conditionMembers: ['마로 엔야', '이연', '페비엘'],
    playableNames:    ['마로 엔야', '이연', '페비엘'],
  },
  {
    id: 'luna2',
    label: '루나 2학년',
    description: '루핀, 카미키, 모나토',
    conditionMembers: ['루핀', '카미키', '모나토'],
    playableNames:    ['루핀', '카미키', '모나토'],
  },
  {
    id: 'luna1',
    label: '루나 1학년',
    description: '아르바로 엔야, 미유, 노노',
    conditionMembers: ['아르바로 엔야', '미유', '노노'],
    playableNames:    ['아르바로 엔야', '미유', '노노'],
  },
  {
    id: 'adventurer',
    label: '대모험가',
    description: '엘네스트, 칼리번, 화이트 팡',
    conditionMembers: ['엘네스트', '칼리번', '화이트 팡'],
    playableNames:    ['엘네스트', '칼리번', '화이트 팡'],
  },
  {
    id: 'outersphere',
    label: 'OuterSphere',
    description: '윤희원, 제로, 크루하',
    conditionMembers: ['윤희원', '제로', '크루하'],
    playableNames:    ['윤희원', '제로', '크루하'],
  },
  {
    id: 'voltex',
    label: 'Voltex Instinct',
    description: '알티 알타, 쿠르쿠마, 커트래빗',
    conditionMembers: ['알티 알타', '쿠르쿠마', '커트래빗'],
    playableNames:    ['알티 알타', '쿠르쿠마', '커트래빗'],
  },
  {
    id: 'fourlords',
    label: '사천왕',
    description: '에코, 카시스, 하피 시아, 리무',
    conditionMembers: ['에코', '카시스', '하피 시아', '리무'],
    playableNames:    ['에코', '카시스', '하피 시아', '리무'],
  },
];

export interface PackProgress {
  pack: RdcPackDef;
  done: number;
  total: number;
  unlocked: boolean;
  active: boolean;
}

/** 팩별 진행도 계산 */
export function getPackProgress(pack: RdcPackDef, actors: Actor[]): { done: number; total: number } {
  const total = pack.conditionMembers.length;
  const done = pack.conditionMembers.filter(name => {
    const actor = actors.find(a => a.name === name);
    return actor ? actor.hyperionLevel >= 5 : false;
  }).length;
  return { done, total };
}

/** 모든 팩의 진행도 + 해금/활성 상태 */
export function getAllPackProgress(actors: Actor[]): PackProgress[] {
  const gs = getGlobalSave();
  return RDC_PACKS.map(pack => {
    const { done, total } = getPackProgress(pack, actors);
    return {
      pack,
      done,
      total,
      unlocked: gs.unlockedRdcPacks.includes(pack.id),
      active: gs.activeRdcPacks.includes(pack.id),
    };
  });
}

/** 조건 달성된 팩을 해금하고 새로 해금된 팩 ID 목록 반환 */
export function checkAndUnlockPacks(actors: Actor[]): string[] {
  const gs = getGlobalSave();
  const newlyUnlocked: string[] = [];
  for (const pack of RDC_PACKS) {
    if (gs.unlockedRdcPacks.includes(pack.id)) continue;
    const { done, total } = getPackProgress(pack, actors);
    if (done >= total) {
      unlockRdcPack(pack.id);
      newlyUnlocked.push(pack.id);
    }
  }
  return newlyUnlocked;
}

/** 현재 활성 팩의 플레이어블 이름 집합 반환 */
export function getActivePackPlayableNames(actors: Actor[]): Set<string> {
  const gs = getGlobalSave();
  const names = new Set<string>();
  for (const pack of RDC_PACKS) {
    if (!gs.activeRdcPacks.includes(pack.id)) continue;
    for (const name of pack.playableNames) {
      if (actors.some(a => a.name === name)) names.add(name);
    }
  }
  return names;
}

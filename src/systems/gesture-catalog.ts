...
export interface GestureDefinition {
  id: string;
  glyph: string;
  /** Spell difficulty tier; a circle counts as four strokes. */
  strokes?: number;
  name: string;
...
export const GESTURE_CATALOG: readonly GestureDefinition[] = [
  ...
  { id:'corner', glyph:'⌞', name:'왼쪽 꺾기', strokes:2, points:points([0,0],[0,1],[1,1]), tolerance:.12 },
  { id:'angle', glyph:'⌝', name:'오른쪽 꺾기', strokes:2, points:points([0,0],[1,0],[1,1]), tolerance:.12 },
  { id:'triangle', strokes:3, glyph:'△', name:'정삼각형', points:points([.5,0],[1,1],[0,1],[.5,0]), closed:true, quick:true, tolerance:.16 },
  { id:'inverted', strokes:3, glyph:'▽', name:'역삼각형', points:points([0,0],[1,0],[.5,1],[0,0]), closed:true, quick:true, tolerance:.16 },
  { id:'circle', strokes:4, glyph:'○', name:'원', points:Array.from({length:65},(_,i)=>({x:.5+.5*Math.cos(i*Math.PI/32),y:.5+.5*Math.sin(i*Math.PI/32)})), closed:true, quick:true, tolerance:.13 },
  ...
  { id:'square', strokes:4, glyph:'□', name:'네모', points:points([0,0],[1,0],[1,1],[0,1],[0,0]), closed:true, tolerance:.12 },
  { id:'diamond', strokes:4, glyph:'◇', name:'마름모', points:points([.5,0],[1,.5],[.5,1],[0,.5],[.5,0]), closed:true, tolerance:.12 },
  { id:'star', strokes:5, glyph:'☆', name:'오각별', points:star, closed:true, drawOnly:true, tolerance:.075, effect:{property:'charge',amount:28,mana:3,reach:4} },
];
